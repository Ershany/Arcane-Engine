...
namespace Arcane
{
	SpotLight::SpotLight(float lightIntensity, glm::vec3 &lightColour, float attenuationRadius, glm::vec3 &pos, glm::vec3 &dir, float innerCutOffAngle, float outerCutOffAngle)
		: Light(lightIntensity, lightColour), m_AttenuationRadius(attenuationRadius), m_Position(pos), m_Direction(dir), m_InnerCutOff(innerCutOffAngle), m_OuterCutOff(outerCutOffAngle) {}
	...
	void SpotLight::SetupUniforms(Shader *shader, int currentLightIndex) {
		...
		shader->SetUniform(("spotLights[" + std::to_string(currentLightIndex) + "].cutOff").c_str(), m_InnerCutOff);
